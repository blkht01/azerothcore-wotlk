...
void Player::PrepareGossipMenu(WorldObject* source, uint32 menuId /*= 0*/, bool showQuests /*= false*/)
{
    ...
    if ((sWorld->getIntConfig(CONFIG_INSTANT_TAXI) == 2 && npcflags & UNIT_NPC_FLAG_FLIGHTMASTER) || ((Player::GetPlayerSetting("store", 3).value != 0) && npcflags & UNIT_NPC_FLAG_FLIGHTMASTER))
...
void Player::OnGossipSelect(WorldObject* source, uint32 gossipListId, uint32 menuId)
{
    ...
    if ((sWorld->getIntConfig(CONFIG_INSTANT_TAXI) == 2 && source->GetTypeId() == TYPEID_UNIT) || (Player::GetPlayerSetting("store", 3).value != 0 && source->GetTypeId() == TYPEID_UNIT))
    {
